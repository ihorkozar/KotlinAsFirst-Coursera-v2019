...
fun lengthInMeters(sagenes: Int, arshins: Int, vershoks: Int):
        Double = 4.445 * 0.001 * (48 * sagenes + 16 * arshins + vershoks)
...
fun angleInRadian(deg: Int, min: Int, sec: Int):
        Double = TODO() //(PI / 180 * 3600) * (sec + min * 60 + deg * 60 * 60)
...
fun accountInThreeYears(initial: Int, percent: Int):
        Double = initial.toDouble() * ((1.0 + percent.toDouble() / 100.0)
        * (1.0 + percent.toDouble() / 100.0)
        * (1.0 + percent.toDouble() / 100.0))
